...
class FileOperations
{
	...
	FileOperations(char *name)
		   ...
		   void displayRecord(int rollNo)                     //to search for a particular student
		   {
			   Student s;
			   file.seekg(0,ios::beg);
			   void *p;
		   ...
		   {
			   ofstream newFile("new.txt",ios::binary);                  //read data from file
			   file.seekg(0,ios::beg);
			   bool flag=false;
			   Student s;
			   while(file.read((char *)&s,sizeof(s)))                     //To read file's data members into an object :

			   {
				   ...
				   newFile.write((char *)&s,sizeof(s));                   //To write object's data members in a file :
